...
* SSH into the app server and check the port on which httpd service is running.

  * Port can be found in the file `sudo vi /etc/httpd/conf/httpd.conf`
* SSH into the LBR host and install nginx server `sudo dnf install nginx -y`
* From LBR host, navigate to file location from the 1st point and update the port to the app server port
* Start the httpd service using systemctl command
* Update the `/etc/nginx/nginx.conf` with the below
...
* Enable and start the nginx service with systemctl commands
* From app server host, do curl operation mentioned to verify the connection